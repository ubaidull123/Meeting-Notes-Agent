...
export function MeetingEditDialog({ isOpen, meeting, initialFocus = 'title', isSaving, onClose, onSave }: MeetingEditDialogProps) {
  ...
  const canEditStructure = meeting.status === 'draft' || meeting.status === 'uploaded';
  const isWorkflowLocked = meeting.status === 'queued' || meeting.status === 'processing';
  const projects = useQuery({ queryKey: ['projects', meeting.team_id], queryFn: () => projectsApi.listProjects(meeting.team_id), enabled: isOpen && canEditStructure });
  const participantOptions = useQuery<MemberOption[]>({ queryKey: ['meeting-participant-options', meeting.team_id, projectId || 'team'], queryFn: async () => projectId ? await projectsApi.listMembers(projectId) : await teamsApi.listMembers(meeting.team_id), enabled: isOpen && canEditStructure });
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (canEditStructure && meeting.restrict_to_participants && !participantIds.length) return setError('Select at least one meeting participant.');
    setError('');
    try {
      await onSave({
        title: title.trim(),
        ...
        ...(canEditStructure ? { project_id: projectId || null, participant_user_ids: participantIds } : {}),
      });
    } catch {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5" role="dialog" aria-modal="true" aria-labelledby="edit-meeting-title">
    ...
    <form onSubmit={submit} className="relative z-10 max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-border bg-card shadow-2xl">
      <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-border bg-card px-4 py-4 sm:px-5">
        <div><h2 id="edit-meeting-title" className="text-lg font-semibold">Meeting settings</h2><p className="mt-1 text-sm text-muted-foreground">Update safe properties for the current workflow state.</p></div>
      ...
      <div className="grid gap-4 p-4 sm:grid-cols-2 sm:p-5">
        ...
        <label className="text-sm font-medium sm:col-span-2">Project<select className={`mt-1.5 ${fieldClass}`} value={projectId} disabled={!canEditStructure} onChange={event => { setProjectId(event.target.value); setParticipantIds([]); }}><option value="">Team-level meeting</option>{projects.data?.map(project => <option key={project.id} value={project.id}>{project.name}</option>)}</select><span className="mt-1.5 block text-xs font-normal text-muted-foreground">{canEditStructure ? 'Changing Project clears the participant selection so it can be validated again.' : 'Project is locked after processing starts.'}</span></label>
      ...
      <div className="mx-4 rounded-lg border border-border p-4 sm:mx-5"><div><h3 className="text-sm font-semibold">Meeting participants</h3><p className="mt-1 text-xs text-muted-foreground">{canEditStructure ? `Only eligible ${projectId ? 'Project' : 'Team'} members can be selected.` : 'Participants are locked after processing starts because they control meeting access.'}</p></div>{canEditStructure ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{participantOptions.data?.filter(member => member.status === 'active' && member.user_id).map(member => { const userId = member.user_id!; const selected = participantIds.includes(userId); return <label key={member.id} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 ${selected ? 'border-primary/30 bg-primary/5' : 'border-border'}`}><input type="checkbox" checked={selected} onChange={() => setParticipantIds(current => selected ? current.filter(id => id !== userId) : [...current, userId])} /><Avatar name={member.full_name} /><span className="min-w-0"><span className="block truncate text-sm font-medium">{member.full_name}</span><span className="block truncate text-xs text-muted-foreground">{member.title || member.department || member.email}</span></span></label>; })}</div> : <div className="mt-3 flex flex-wrap gap-2">{meeting.attendees.map(participant => <span key={participant.id ?? participant.email} className="rounded-full border border-border bg-muted/40 px-2.5 py-1 text-xs font-medium">{participant.name}</span>)}</div>}</div>
      {isWorkflowLocked && <p className="mx-4 mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800 sm:mx-5">Meeting edits are locked while processing is active.</p>}
      ...
      <div className="sticky bottom-0 mt-5 flex flex-col-reverse gap-2 border-t border-border bg-card px-4 py-3 sm:flex-row sm:justify-end sm:px-5">
        ...
        <button type="submit" disabled={isSaving || isWorkflowLocked} className={primaryButton}>{isSaving && <Loader2 className="h-4 w-4 animate-spin" />}Save changes</button>
      </div>
    </form>
  </div>;
}
