...
import { CalendarDays, Clock3, FileAudio, FileText, Loader2, Plus, Search, Upload, Users } from 'lucide-react';
...
export function MeetingsWorkspacePage() {
  ...
  const meetingQuery = useQuery({ queryKey: ['meetings', activeTeam?.id, status], queryFn: () => meetingsApi.listMeetings({ team_id: activeTeam!.id, status: (status || undefined) as MeetingStatus | undefined, page_size: 100 }), enabled: Boolean(activeTeam) });
  if (!activeTeam) return <EmptyState title="No active team" description="Select a team to view meetings." />;
  const searchTerm = search.trim().toLowerCase();
  const meetings = {
    ...meetingQuery,
    data: (meetingQuery.data ?? []).filter(meeting => `${meeting.title} ${meeting.project_name ?? ''} ${meeting.created_by_name ?? ''}`.toLowerCase().includes(searchTerm)),
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_15rem]"><label className="relative"><span className="sr-only">Search meetings</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input className={`${fieldClass} pl-9`} value={search} onChange={event => setSearch(event.target.value)} placeholder="Search meetings or projects" /></label><label><span className="sr-only">Workflow status</span><select aria-label="Filter meetings by status" className={fieldClass} value={status} onChange={event => setStatus(event.target.value)}><option value="">All statuses</option>{meetingStatuses.map(value => <option key={value} value={value}>{value.replace(/_/g, ' ')}</option>)}</select></label></div>
...
export function CreateMeetingWorkspacePage() {
  ...
  const [inputType, setInputType] = useState<'later' | 'text' | 'transcript' | 'audio'>('later');
  ...
  const submit = (event: FormEvent) => {
    ...
    if (inputType === 'text' && !form.transcript_text.trim()) { setError('Paste the transcript or choose Add source later.'); return; }
    if ((inputType === 'transcript' || inputType === 'audio') && !file) { setError('Choose a file or choose Add source later.'); return; }
